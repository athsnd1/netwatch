import { ROUTER_MIB } from "../../../constants/mibs/router.mib.js";
 ...
 *
 * We only map the common ones. Unknown values are returned
 * as "UNKNOWN".
 */
...
/*
 * Convert IF-MIB ifType numeric value into something
 * meaningful to NetWatch.
 */
...
/*
 * Convert interface speed from bits/second to Mbps.
 *
 * Example:
...
/*
 * Retrieve all interfaces exposed through IF-MIB.
 */
export async function getRouterInterfaces(
  ...
  const results = await walkSNMP(
    address,
    ROUTER_MIB.interfaces.base,
    community
  );

  const interfaces = new Map<number, RouterInterface>();
  ...
  for (const vb of results) {
    ...
    */
    ...
    switch (column) {
       ...
       */
      case 3:
        networkInterface.type = normalizeInterfaceType(
          vb.value
        );
        break;
      ...
      case 5: {
        ...
        networkInterface.speedMbps = normalizeSpeed(
          speed
        );
      ...
      case 7:
        networkInterface.adminStatus =
          normalizeInterfaceStatus(vb.value);
        break;
...
/*
 * Retrieve CPU usage from HOST-RESOURCES-MIB.
 *
 * hrProcessorLoad is a table because a device may have
 * multiple processors.
 *
 * We therefore calculate the average processor load.
 */
export async function getRouterCPUUsage(
  address: string,
  community = "public"
): Promise<number | null> {
  try {
    const results = await walkSNMP(
      address,
      ROUTER_MIB.resources.processor.base,
      community
    );
    ...
    /*
     * Router may not support HOST-RESOURCES-MIB.
     */
    return null;
  }
}
...
/*
 * Retrieve memory information from HOST-RESOURCES-MIB.
 *
 * hrStorageTable contains several storage areas, so we
 * identify the physical memory entry using hrStorageType.
 */
export async function getRouterMemory(
  address: string,
  community = "public"
): Promise<{
  memoryTotal: number | null;
  ...
  try {
    const results = await walkSNMP(
      address,
      ROUTER_MIB.resources.storage.base,
      community
    );

    /*
     * hrStorageType value for hrStorageRam:
     *
     * 1.3.6.1.2.1.25.2.1.2
     */
    const HR_STORAGE_RAM =
      "1.3.6.1.2.1.25.2.1.2";
    ...
    for (const vb of results) {
       ...
       * column 4 = hrStorageAllocationUnits
       * column 5 = hrStorageSize
       * column 6 = hrStorageUsed
       *
       * index is the final OID component.
       */
    ...
    /*
     * hrStorageSize and hrStorageUsed are measured in
     * allocation units.
     */
    const totalBytes =
      memory.size * memory.allocationUnits;

    const usedBytes =
      memory.used * memory.allocationUnits;
...
export async function getRouterResources(
  ...
  const [cpuUsage, memory] = await Promise.all([
    getRouterCPUUsage(address, community),
    getRouterMemory(address, community),
  ]);
...
/*
 * Retrieve the complete router metric set.
 */
export async function getRouterMetrics(
  ...
  const [
    ...
    checkSNMP(
      address,
      ROUTER_MIB.system.description,
      community
    ),

    checkSNMP(
      address,
      ROUTER_MIB.system.objectId,
      community
    ),

    checkSNMP(
      address,
      ROUTER_MIB.system.uptime,
      community
    ),

    checkSNMP(
      address,
      ROUTER_MIB.system.name,
      community
    ),

    checkSNMP(
      address,
      ROUTER_MIB.system.location,
      community
    ),

    getRouterInterfaces(address, community),

    getRouterResources(address, community),
  ]);
  ...
  return {
    ...
    uptime:
      uptime.value !== undefined
        ? toNumberOrNull(uptime.value)
        : null,
